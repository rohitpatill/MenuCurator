...
| `app/menu.py` | 128 | **Menu data access.** Loads `menu.json` once at import, exposes `get_dish`, `available_dishes`, `apply_filters` (the deterministic chip-filter logic — **food-type dishes only**; drinks never enter the food pool here), `drink_candidates` (mocktail/cocktail/both per the Drinks chip), and `compact` (slim dish shape for prompts to save tokens). |
| `app/menu.json` | — | **The entire menu** (~128 dishes). Static data, no DB. Each dish carries all filterable metadata. A couple are marked `available:false` to exercise sold-out handling. |
...
| `src/constants.js` | 53 | `FILTER_GROUPS` (the 7 chip filters), `EXTRA_CHIPS` (Show-more filters), `DEFAULT_FILTERS`, `partySize(party, custom)` (maps the party chip to a head-count; returns the custom value when "Custom" is selected), and the suggested-question lists for dish Q&A and refine. |
