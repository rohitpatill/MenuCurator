// Shared style tokens + tiny presentational atoms used across screens.
...
export function chipStyle(selected) {
  ...
  return selected
    ? { ...base, border: "1px solid #1C1714", background: "#1C1714", color: "#FBF6EC", boxShadow: "0 5px 14px -6px rgba(28,23,20,.5)" }
    : { ...base, border: "1px solid #E4DCCD", background: "#FFFEFB", color: "#5C5448", fontWeight: 500 };
}
...
export function maitreIcon(size = 42, radius = 13) {
  return {
    flex: "none",
    width: size,
    height: size,
...
export function Spark({ size = 16, fill = "#fff" }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none">
      <path d="M12 2.5l1.9 5.4 5.6.2-4.4 3.5 1.5 5.4L12 19.3 7.9 22.4l1.5-5.4L5 13.5l5.6-.2L12 2.5z" fill={fill} />
    </svg>
  );
}
...
export function Bubble({ m }) {
  return (
    <div style={{ display: "flex", justifyContent: m.role === "user" ? "flex-end" : "flex-start", marginBottom: 8 }}>
...
export function Typing() {
  return (
    <div style={{ display: "flex", justifyContent: "flex-start", marginTop: 4 }}>
      <div style={{ display: "flex", gap: 5, padding: "13px 16px", background: "#FFFEFB", border: "1px solid #ECE4D4", borderRadius: "16px 16px 16px 4px" }}>
        {[0, 0.18, 0.36].map((d, i) => (
          <span key={i} style={{ width: 7, height: 7, borderRadius: "50%", background: "#E8662B", animation: `vmBlink 1.1s infinite ${d}s` }} />
        ))}
      </div>
    </div>
  );
